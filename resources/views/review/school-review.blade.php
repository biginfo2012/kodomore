...
@section('content')
    ...
    <div class="card">
        <div class="card-header title-background text-small-xs d-flex ">
            ...
            <p><span id="move_my_page" class="top-title-tag">ログイン </span> > <span id="" class="top-title-tag">マイページ</span> > クチコミレビューに返事をする</p>

        </div>

        <div class="card-body text-title pb-2 pr-1">
            <img class="height-4 img-icon mb-1" src="{{asset('img/back-icon.png')}}">クチコミレビューに返事をする(2回目)
        </div>

        <div class="card-body py-1">
            <p class="text-title background-dark-blue text-white p-1 mt-0">投稿内容</p>
        ...
        <div class="card-body">
            <form class="needs-validation" name="complete" novalidate id="validation_form" method="post" action="{{url('/replay/school/view')}}">
                {{csrf_field()}}

                <input type="hidden" name="post_user_id" value="{{$post_user_id}}">
                <input type="hidden" name="review_id" value="{{$review->id}}">
                <input type="hidden" name="post_order" value="3">
                ...
                <div class="mt-2 custom-control custom-radio">
                    <input type="radio" class="custom-control-input" id="radio_accept1" name="radio_accept" value="1" checked>
                    <label class="custom-control-label text-medium-xs" for="radio_accept1">登録画像を公開表示する</label>
                </div>

                <div class="mt-2 custom-control custom-radio">
                    <input type="radio" class="custom-control-input" id="radio_accept2" name="radio_accept" value="0">
                    <label class="custom-control-label text-medium-xs" for="radio_accept2">非公開表示にする</label>
                </div>
