...
@section('content')
    ...
    <div class="card">
        <div class="card-header title-background text-small-xs d-flex ">
            ...
            <p><span id="move_my_page" class="top-title-tag">ログイン </span> > <span id="" class="top-title-tag">マイページ</span> > クチコミレビュー削除依頼</p>

        </div>

        <div class="card-body text-title">
            <img class="height-4 img-icon mb-1" src="{{asset('img/delete-black.png')}}"> クチコミレビュー削除依頼
        </div>
        ...
        <div class="card-body py-1">
            ...
            <div class="mt-2 text-3 text-small">
                <p>･クチコミレビュー削除は取り消しができません｡
                </p>
            </div>
            <form class="needs-validation" name="complete" novalidate id="validation_form" method="post" action="{{url('/delete/finish/review')}}">
                {{csrf_field()}}

                    <input type="hidden" class="custom-control-input" id="radio_accept_5" name="review_id" value="{{$review_id}}">
                ...
                <div class="mt-4 flex justify-content-center">
                    <button type="submit" class="gray-btn-gradient mx-0 btn btn-outline-default rounded waves-effect btn-full btn-title text-white" id="btn_submit" disabled>削除依頼を送信する</button><i class="text-white fas fa-angle-right ml-1"></i>
                </div>
            </form>
