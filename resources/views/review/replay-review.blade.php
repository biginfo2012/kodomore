...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body text-title pb-2 pr-1">
            <img class="height-4 img-icon mb-1" src="{{asset('img/back-icon.png')}}">クチコミレビューに返事をする(1回目)
        </div>
        ...
        <div class="card-body">
            <form class="needs-validation" name="complete" novalidate id="validation_form" method="post" action="{{url('/replay/view')}}">
                {{csrf_field()}}

                <input type="hidden" name="post_user_id" value="{{$post_user_id}}">
                <input type="hidden" name="review_id" value="{{$review->id}}">
                <input type="hidden" name="post_order" value="1">
