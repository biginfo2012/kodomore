...
@section('content')
    <div class="card">
        <div class="card-header title-background text-small-xs d-flex ">
            ...
            <p><span class="top-title-tag">ログイン </span> > <span id="move_my_page" class="top-title-tag">マイページ </span> > 資料請求歴</p>
        </div>

        <div class="card-body text-large-medium-xs py-2 background-white-blue" style="border-bottom: 1px solid #ABABAB">
            <img class="height-2 img-icon py-1" src="{{asset('img/data-list.png')}}"> 資料請求歴
            ...
            <div class="float-right sort-select">
                <select class="px-2 py-1 custom-select custom-select-sm form-control form-control-sm text-small" name="old">
                    <option value="new" selected>資料請求/新しい順</option>
                    <option value="old">資料請求/古い順</option>
                </select>
            </div>
        </div>
...
@section('js4event')
    <script language="javascript" type="text/javascript">
        ...
        function getSchoolList(order) {
            ...
            var url = home_path + '/parent/data/get-list';
            $.ajax({
                url:url,
                type:'post',
                data: {
