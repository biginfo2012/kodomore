...
@section('content')
    <div class="card">
        <div class="card-header title-background text-small-xs d-flex ">
            ...
            <p><span class="top-title-tag">ログイン </span> > <span id="move_my_page" class="top-title-tag">マイページ </span> > メッセージ一覧</p>
        </div>

        <div class="card-body text-large-medium-xs py-2 background-white-blue" style="border-bottom: 1px solid #ABABAB">
            <img class="height-2 img-icon py-1" style="padding: 6px 0 7px 3px !important;" src="{{asset('img/mail.png')}}"> メッセージ一覧
            ...
            <div class="float-right sort-select">
                <select class="px-2 py-1 custom-select custom-select-sm form-control form-control-sm text-small" name="old">
                    <option value="time" selected>送信日時順</option>
                    <option value="read">未読のみ</option>
                </select>
            </div>
        </div>

        <div class="card" id="school_list">
...
@section('js4event')
    <script language="javascript" type="text/javascript">
        ...
        function getSchoolList(order) {
            ...
            var url = home_path + '/parent/mail/get-list';
            $.ajax({
                url:url,
                type:'post',
                data: {
