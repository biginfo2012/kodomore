...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body py-1">
            ...
            <div class="background-light-gray my-1">
                <p class="text-small p-1">
                    園名
                </p>
            </div>
            @foreach($garden_details -> old as $garden_detail)
                <p class="mt-1 text-medium">
                    <?=$garden_detail -> info->prefecture_name;?> <?=$garden_detail -> info->city_name;?>
                    @foreach($garden_detail -> info -> typeList as $type)
                        ｜ {{$type->type_name}}
                    @endforeach
                    | {{$garden_detail -> info -> garden_name}}
                </p>
            @endforeach

            @foreach($garden_details -> cur as $garden_detail)
                <p class="mt-1 text-medium">
                    <?=$garden_detail -> info->prefecture_name;?> <?=$garden_detail -> info->city_name;?>
                    @foreach($garden_detail -> info -> typeList as $type)
                        ｜ {{$type->type_name}}
        ...
        <form id="register_form" action="<?=url('/register/complete');?>" method="post">
            ...
            <input type="hidden" name="garden_info" value="{{$garden_info}}">
