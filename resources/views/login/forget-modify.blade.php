...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body text-title">
            <img class="height-4 img-icon mb-1" src="{{asset('img/forget-icon.png')}}">  パスワードを再設定
        </div>

        <div class="card-body py-1">
            <p class="text-medium">新しいパスワードを作成し､再設定してください｡半角英文字､半角数字がご利用いただけます｡</p>
            <form class="needs-validation" novalidate id="register_form">
                ...
                <p  class="mt-1 form-control text-small "   name="email" >{{$email}}</p>
                ...
                <div class="py-1 mt-4">
                    ...
                    <div class="mt-1 flex justify-content-center">
                        <button class="gray-btn-gradient mx-0 btn btn-outline-default rounded waves-effect btn-full btn-title text-white" id="btn_register" style="border: 0 !important;">パスワードを再設定する</button><i class="text-white fas fa-angle-right ml-1"></i>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if(response['status'] == true) {
                                    window.location.href = home_path + '/forget/final'
                                } else {
                                    alertify.error("変更に失敗しました");
                                }
