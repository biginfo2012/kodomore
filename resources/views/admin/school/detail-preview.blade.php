...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body d-none" id="notification">
        </div>
        ...
        <div id="logo" class="card-header text-center text-large">
        </div>
        ...
        <div class="card-body py-1">
            ...
            <div class="d-flex flex-wrap mt-2">
                @foreach($tagList as $index => $tag)
                    <div class=" mr-3 mb-2 position-relative">
                        <div class="title-background py-1 px-2">
                            <div class=" tag-info text-small"><?=$tag->tag_title;?></div>
                        </div>
                    </div>
                @endforeach
        ...
        <div class="card-body py-0">
            ...
            <div class="text-title-large text-black" id="comment_title" style="white-space: pre-line"></div>
            <hr class="dot-border">
            <div class="text-break text-medium-light "
                 style="text-align: justify;white-space: pre-line" id="comment_description"></div>
        </div>
            ...
            <img class="" src="{{asset('img/kodomore-snap.png')}}">
                ...
                <div class="card-body" id="map">
                    ...
                    <div id="map-container-google-2" class="z-depth-1-half map-container mt-1">
                        <iframe class="d-none" id="d_frame"
                                src="https://maps.google.com/maps?q={{$garden->prefecture_name.' '.$garden->city_name.' '.$garden->town_name.' '.$garden->address}}&t=&z=13&ie=UTF8&iwloc=&output=embed"
        ...
        @section('js4event')
            ...
            <script language="javascript" type="text/javascript">
                ...
                $(document).ready(function () {
                    var garden_info = JSON.parse(localStorage.getItem("garden_preview"));
                    console.log(garden_info);
                    if(garden_info['status'] !== ''){
                        $('#status').text(garden_info['status']);
                    }
                    else{
                        $('#status').addClass('d-none');
                    }

                    var keywordList = garden_info['keywordList'];
                    for(var i =0; i < keywordList.length; i++){
                        var content = '<div class=" mr-3 mb-2 px-2 py-1 keyword-background text-small rounded rounded-small">\n' +
                            keywordList[i] + '</div>';
                        $('#keywordList').append(content);
                    }


                    if(garden_info['free_edu'] === '1'){
                        var content = '<div class=" mr-3 mb-2 px-2 py-1 keyword-background-active text-small rounded rounded-small">幼児教育･保育無償化</div>';
                        $('#keywordList').append(content);
                    }

                    $('#comment_title').text(garden_info['comment_title']);
                    $('#comment_description').text(garden_info['comment_description']);

                    var map_setting = garden_info['map_setting'];
                    if(map_setting === '2'){
                        $('#map').addClass('d-none');
                    }
                    else{
                        $('#map-container-google-2').append(garden_info['map_iframe']);
                    }
                    '<img class="logo-big" src="{{asset('/storage/img/garden/'.$garden->photo)}}">'
                    if(garden_info['logo_type'] == 1){
                        var content =  '<img class="logo" src="' + garden_info['imgLogo'] + '">'

                    }else if(garden_info['logo_type'] == 2){
                        var content =  '<img class="logo-big" src="' + garden_info['imgLogo'] + '">'
                    }
                    $('#logo').append(content);

                    var notificationList = garden_info['notificationList'];
                    for(var i = 0; i < notificationList.length; i++){
                        if(notificationList[i].type !== undefined && notificationList[i].content !== undefined){
                            if(notificationList[i].type === '1' && notificationList[i].content !== ''){
                                var content = '<div class="justify-content-center text-center px-2 mb-2" style="border: 1px solid #ED1C24">\n' +
                                    '                <p class="text-large text-left line-clamp1" style="color: #ED1C24">\n' +
                                    notificationList[i].content +
                                    '                </p>\n' +
                                    '                <img class="rotate-icon height-1 img-icon whole_text_show"\n' +
